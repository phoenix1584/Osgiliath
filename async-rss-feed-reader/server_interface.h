    ...
    namespace networkcore{
        ...
        class ServerInterface{
            public:
                ...
                void MessageClient(std::shared_ptr<T> client, const Message<T>& msg){
                    if(client && client->IsConnected()){
                        client->Send(msg);
                    }else{
                        OnClientDisconnect(client);
                        client.reset();
                        m_conn_queue.erase(std::remove(m_conn_queue.begin(), m_conn_queue.end(),client),m_conn_queue.end());
                    }
                }
                
                void Broadcast(const Message<T>& msg,std::shared_ptr<Connection<T>> ignore_client = nullptr){
                    ...
                    if(invalid_client_exists)
                        m_conn_queue.erase(std::remove(m_conn_queue.begin(), m_conn_queue.end(),nullptr),m_conn_queue.end());
                }
            ...
            protected:
                ...
                virtual bool OnClientDisconnect(std::shared_ptr<Connection<T>> client){ return false; }
                virtual bool OnMessage(std::shared_ptr<Connection<T>> client, Message<T>& msg){ return false; }
            protected:
                ...
                uint32_t m_client_identifier = 12345; // TODO: This has to be random for multiserver.
